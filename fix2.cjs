...
// 5. Replace inline object in loops outside of xy
content = content.replace(
  /const res = f\.compiled\.evaluate\(\{ \.\.\.baseScope, t: tVal \}\);/g,
  'loopScope.t = tVal; const res = f.compiled.evaluate(loopScope);'
);
content = content.replace(
  /const res = f\.compiled\.evaluate\(\{ \.\.\.baseScope, x: xVal \}\);/g,
  'loopScope.x = xVal; const res = f.compiled.evaluate(loopScope);'
);
// Make sure to add loopScope = { ...baseScope } before those for-loops!

fs.writeFileSync('src/components/MathNodeRenderer.tsx', content);
console.log('Fixed fix2');
