...
export function TsNodeCodeRenderer({ code, path, width, height }: TsNodeCodeRendererProps) {
  ...
  useEffect(() => {
    if (monaco) {
      try {
        ...
        console.warn("Failed to configure Monaco compiler isolation settings in TsNodeCodeRenderer", err);
      }
    }
  }, [monaco]);
         ...
         for (let entry of entries) {
            ...
            if (width > 0 && height > 0) {
               window.requestAnimationFrame(() => {
                  setCustomNodeSize(path + '.__ts_code', width, height);
               });
            }
         }
       });
      ...
      >
        ...
        <div className="flex items-center justify-between p-2 border-b border-[#30363d]/50 bg-[#0d1218] drag-handle cursor-move">
           <div className="flex items-center gap-1.5 px-1">
             <Code2 size={12} className="text-[#3178C6]" />
             <span className="text-[10px] font-semibold text-slate-300 uppercase tracking-widest">TS Code</span>
           </div>
           <div className="flex items-center gap-1">
             <button
                onClick={(e) => {
                   ...
                   if (isLoading) {
                      ...
                      executeTsNode(path, localCode, monaco);
                   }
                }}
                className={`flex items-center justify-center gap-1.5 py-0.5 px-2 font-medium text-[10px] rounded transition-colors ${isLoading ? "bg-slate-700 hover:bg-slate-600 text-white" : "bg-[#3178C6] hover:bg-[#2762a4] text-white"}`}
             ...
             <button
                ...
                className="flex items-center justify-center p-1 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 rounded transition-colors"
