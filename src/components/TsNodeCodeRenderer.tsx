...
export function TsNodeCodeRenderer({
  code,
  path,
  width,
  height,
  ...
  useEffect(() => {
    if (monaco) {
      try {
        ...
        console.warn(
          "Failed to configure Monaco compiler isolation settings in TsNodeCodeRenderer",
          err,
        );
      }
    }
  ...
  useEffect(() => {
    if (jsNodeFocusLine?.path === path && editorRef.current) {
      if (!isExpanded) {
        ...
        setTimeout(() => setJsNodeFocusLine(null), 100);
      }
    }
  }, [
    jsNodeFocusLine,
  ...
  const updateGlobalCode = React.useCallback(
    (newCode: string) => {
      ...
      >
        ...
        {isGoToLineOpen && (
          <div className="absolute top-2 right-2 z-50 bg-[#161b22] border border-[#30363d]/50 shadow-xl rounded-md p-1.5 flex items-center gap-1.5 animate-in fade-in zoom-in-95 duration-100">
            <span className="text-[10px] font-bold text-slate-400 font-mono pl-1 shrink-0">
              Go to:
            </span>
            <input
              type="text"
