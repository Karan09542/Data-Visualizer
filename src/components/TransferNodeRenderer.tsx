...
import { Copy, Plus, Video, Image as ImageIcon, Send, MessageSquare, X, RefreshCw, UploadCloud, Download, Check, File, Globe, Share2, Laptop, Smartphone, Terminal, History, ChevronRight, Activity } from "lucide-react";
...
export const TransferNodeRenderer: React.FC<{
  ...
  const sendLargeMessage = async (type: string, payloadStr: string, fileName?: string) => {
      ...
      try {
        ...
        if (msg.type === "text") {
          ...
          if (chunkData) {
            ...
            chunkData.count++;
            setTransferProgress(Math.floor((chunkData.count / chunkData.total) * 100));
            ...
            if (chunkData.count === chunkData.total) {
               ...
               if (chunkData.type === "workspace") {
                 try {
                  ...
                  setMessages(prev => [...prev, { id: uuidv4(), sender: "remote", type: "file", fileName: chunkData.fileName, content: fullPayload, timestamp: Date.now() }]);
               }
            }
          }
  ...
  const sendWorkspace = () => {
    ...
    setConnectionState("transferring");
    sendLargeMessage("workspace", wsStr).then(() => setConnectionState("connected"));
    ...
    <div className={`w-[400px] min-h-[340px] rounded-2xl overflow-hidden border shadow-2xl flex flex-col ${isDark ? "bg-[#111829] border-white/10 text-slate-300" : "bg-white border-slate-200 text-slate-800"}`}>
      ...
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className={`font-bold tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>Direct Transfer</h3>
        ...
        <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
          connectionState === "connected" ? "bg-emerald-500/10 text-emerald-500" :
          connectionState === "transferring" ? "bg-blue-500/10 text-blue-500" :
      ...
      <div className="flex-1 px-5 pb-5 flex flex-col">
        {connectionState === "waiting" && !scanMode && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <p className="text-sm leading-relaxed text-slate-400">
              Transfer nodes, documents, or your entire workspace instantly between devices using WebRTC. No accounts required.
            </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className={`p-6 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 transition-all group ${
                  isDark ? "border-white/5 hover:border-indigo-500/50 hover:bg-indigo-500/5" : "border-slate-200 hover:border-indigo-500 hover:bg-indigo-50"
                }`}
              >
                <div className={`p-3 rounded-full transition-colors ${isDark ? "bg-white/5 group-hover:bg-indigo-500/20" : "bg-slate-100 group-hover:bg-indigo-100"}`}>
                  <Laptop className="w-6 h-6" />
                </div>
                <span className="text-xs font-bold tracking-tight">Host Transfer</span>
              </button>

              <button 
                onClick={() => setScanMode("offer")}
                className={`p-6 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 transition-all group ${
                  isDark ? "border-white/5 hover:border-emerald-500/50 hover:bg-emerald-500/5" : "border-slate-200 hover:border-emerald-500 hover:bg-emerald-50"
                }`}
              >
                <div className={`p-3 rounded-full transition-colors ${isDark ? "bg-white/5 group-hover:bg-emerald-500/20" : "bg-slate-100 group-hover:bg-emerald-100"}`}>
                  <Smartphone className="w-6 h-6" />
                </div>
                <span className="text-xs font-bold tracking-tight">Join / Scan</span>
              </button>
            </div>
          </motion.div>
        )}
        ...
        {connectionState === "pairing" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-6 py-2">
            ...
            <div className="text-center space-y-1">
              <p className="text-sm font-bold text-white">Device Pairing Required</p>
              <p className="text-xs text-slate-500">Scan this code on the other device to link</p>
            </div>

            <div className="w-full flex flex-col gap-2">
              <div className={`flex gap-2 p-1.5 rounded-xl border ${isDark ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-100"}`}>
                <input 
                  type="text" 
                  placeholder={offerQR ? "Paste answer code..." : "Paste offer code..."}
                  className="flex-1 bg-transparent text-xs px-3 focus:outline-none" 
                  value={offerQR ? copyPasteAnswer : copyPasteOffer} 
                  onChange={e => offerQR ? setCopyPasteAnswer(e.target.value) : setCopyPasteOffer(e.target.value)} 
                />
                <button 
                  onClick={() => handleScan(offerQR ? copyPasteAnswer : copyPasteOffer)}
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => navigator.clipboard.writeText(offerQR || answerQR)}
        ...
        {(connectionState === "connected" || connectionState === "transferring") && (
          <div className="flex flex-col h-[320px]">
            {/* Messages Area */}
            <div className={`flex-1 overflow-y-auto mb-4 space-y-4 pr-1 scrollbar-thin ${isDark ? "scrollbar-dark" : "scrollbar-light"}`}>
              <AnimatePresence initial={false}>
                {messages.length === 0 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center text-center px-8 gap-4">
                    <div className={`p-4 rounded-full ${isDark ? "bg-white/5 text-slate-600" : "bg-slate-50 text-slate-400"}`}>
                      <MessageSquare className="w-10 h-10" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-400">Direct Link Active</p>
                      <p className="text-xs text-slate-500 mt-1">Ready to transfer files, nodes, and workspace data</p>
                    </div>
                  </motion.div>
                )}
                {messages.map(msg => (
                  ...
                  >
                    <div className={`group relative max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-sm transition-all ${
                      msg.sender === "me" 
                        ? "bg-indigo-600 text-white rounded-tr-none" 
                        : isDark ? "bg-[#161f30] text-slate-200 rounded-tl-none border border-white/5" : "bg-slate-100 text-slate-800 rounded-tl-none"
                    }`}>
                      {msg.type === "text" ? msg.content : (
                        <div className="flex items-center gap-3 py-1">
                          <div className={`p-2 rounded-lg ${msg.sender === "me" ? "bg-white/20" : isDark ? "bg-indigo-500/10 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}>
                            <File className="w-5 h-5" />
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="text-[11px] font-bold truncate leading-none mb-1">{msg.fileName}</span>
                            <span className="text-[9px] opacity-70 uppercase tracking-tighter">Received File</span>
                          </div>
                        </div>
                      )}
                    </div>
                    <span className="text-[9px] text-slate-500 mt-1.5 px-1 font-medium">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-1.5 px-1">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                    <RefreshCw className="w-3 h-3 animate-spin" /> {connectionState === "transferring" ? "Syncing Workspace..." : "Sending File..."}
                  </span>
                  <span className="text-[10px] font-bold text-slate-500">{transferProgress}%</span>
                </div>
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <button 
                  ...
                  className={`flex-1 flex gap-2 items-center justify-center py-3 rounded-xl text-[10px] font-bold uppercase transition-all border ${
                    isDark ? "border-white/5 bg-white/5 hover:bg-white/10 hover:border-indigo-500/30" : "border-slate-100 bg-slate-50 hover:bg-indigo-50 hover:border-indigo-200"
                  } disabled:opacity-50`}
                >
                  <Globe className="w-3.5 h-3.5 text-indigo-500" /> Sync Project
                </button>
                <label className={`flex-1 flex gap-2 items-center justify-center py-3 rounded-xl text-[10px] font-bold uppercase transition-all border cursor-pointer ${
                  isDark ? "border-white/5 bg-white/5 hover:bg-white/10 hover:border-emerald-500/30" : "border-slate-100 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-200"
                }`}>
                  <UploadCloud className="w-3.5 h-3.5 text-emerald-500" /> Send Assets
              ...
              <div className="flex items-center gap-2">
                <div className={`flex-1 flex items-center rounded-xl p-1 border transition-all ${
                  isDark ? "bg-[#161f30] border-white/5 focus-within:border-indigo-500/50" : "bg-slate-50 border-slate-100 focus-within:border-indigo-500"
                }`}>
                  <input 
                    type="text" 
                    value={chatInput}
