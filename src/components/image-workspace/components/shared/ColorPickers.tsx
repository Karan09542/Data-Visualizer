...
export const ColorPickerPortal = ({ color, onChange, onClose, anchorRef }: any) => {
   ...
   return createPortal(
      <div 
         ref={popoverRef} 
         className="absolute p-3 bg-[#1C1C1C] border border-[#333] rounded-xl shadow-2xl flex flex-col gap-3 transition-opacity duration-100 z-[999999]"
         style={{ 
            top: coords.top, 
            left: coords.left, 
         ...
         <div className="flex items-center gap-2 mt-0.5">
            ...
            <div className="flex-1 relative">
               <input 
                  ...
                  className="bg-[#121212] border border-[#333] rounded-md px-2.5 py-1.5 text-[11px] text-slate-300 font-mono w-full focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 transition-all" 
               />
            </div>
            {isEyeDropperSupported && (
               <button 
                  ...
                  className="w-7 h-7 bg-[#2A2A2A] hover:bg-[#3A3A3A] text-slate-300 hover:text-white rounded-md border border-[#333] hover:border-[#444] transition-all active:scale-95 shrink-0 flex items-center justify-center shadow-sm"
