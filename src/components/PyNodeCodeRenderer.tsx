...
interface PyNodeCodeRendererProps {
  code: string;
...
export function PyNodeCodeRenderer({ code, path, width, height }: PyNodeCodeRendererProps) {
  ...
  useEffect(() => {
    if (monaco) {
      try {
        ...
        console.warn("Failed to configure Monaco compiler isolation settings in PyNodeCodeRenderer", err);
      }
    }
  }, [monaco]);
      ...
      >
        ...
        <div className="flex items-center justify-between p-2 border-b border-[#30363d]/50 bg-[#0d1218] drag-handle cursor-move">
           <div className="flex items-center gap-1.5 px-1">
             ...
             <span className="text-[10px] font-semibold text-slate-300 uppercase tracking-widest">PY Code</span>
           </div>
             ...
             <button
                onClick={(e) => {
                   ...
                   if (isLoading) {
                      ...
                      executePyNode(path, localCode);
                   }
                }}
                className={`flex items-center justify-center gap-1.5 py-0.5 px-2 font-medium text-[10px] rounded transition-colors ${isLoading ? "bg-slate-700 hover:bg-slate-600 text-white" : "bg-[#10b981] hover:bg-[#2762a4] text-white"}`}
