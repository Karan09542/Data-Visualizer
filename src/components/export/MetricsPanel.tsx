...
export const MetricsPanel: React.FC<Props> = ({
  ...
  return (
    <div className="bg-[#141414] border border-[#222] rounded-lg md:rounded-xl p-2 md:p-4 space-y-2 md:space-y-4 shadow-inner">
      <div className="flex items-center justify-between border-b border-[#222] pb-1.5 md:pb-2 mb-1.5 md:mb-2">
        ...
        <span className="text-[8px] md:text-[9px] font-bold font-mono text-slate-400 uppercase bg-[#1A1A1A] border border-[#2A2A2A] px-1.5 md:px-2 py-0.5 rounded-full">
          {format.toUpperCase()}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-4">
        ...
        <div className="space-y-1.5 md:space-y-3">
          <div className="space-y-0.5">
            ...
            <div className="text-xs md:text-sm font-mono text-slate-300">{formatBytes(originalSize)}</div>
            <div className="text-[9px] md:text-[10px] text-slate-600 font-mono">{originalWidth} × {originalHeight}</div>
          </div>
          <div className="space-y-0.5">
            <div className="flex items-center gap-1.5">
               <div className="text-[8px] md:text-[9px] text-blue-400 uppercase font-bold tracking-tighter">Optimized Export</div>
               {isSavings ? (
                  reduction > 0 && <span className="text-[7.5px] md:text-[8px] font-mono font-bold px-1 py-0.5 md:px-1.5 md:py-0.2 bg-blue-900/40 text-blue-300 rounded md:rounded-md border border-blue-500/20">-{reduction.toFixed(1)}%</span>
               ) : (
                  <span className="text-[7.5px] md:text-[8px] font-mono font-bold px-1 py-0.5 md:px-1.5 md:py-0.2 bg-indigo-900/40 text-indigo-300 rounded md:rounded-md border border-indigo-500/20">HQ Native</span>
               )}
            </div>
            <div className="text-base md:text-lg font-mono font-black text-white">{formatBytes(optimizedSize)}</div>
            <div className="text-[9px] md:text-[10px] text-slate-400 font-mono">{optimizedWidth} × {optimizedHeight}</div>
          </div>
        </div>
      ...
      <div className="mt-2 md:mt-4 p-2 md:p-3 bg-slate-900/50 rounded-lg border border-slate-800/50">
        <div className="flex items-center justify-between text-[9px] md:text-[10px]">
