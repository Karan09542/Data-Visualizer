...
export const ExportStudio: React.FC<Props> = ({
  ...
  const setPreset = (presetName: string) => {
    ...
    switch (presetName) {
      ...
      case 'Thumbnail':
        newSettings.format = 'webp';
        newSettings.webp.quality = 60;
        newSettings.resize.enabled = true;
        newSettings.resize.width = 150;
        newSettings.resize.maintainAspectRatio = true;
        break;
      case 'Discord/Chat':
        newSettings.format = 'webp';
        newSettings.webp.quality = 70;
        newSettings.resize.enabled = true;
        newSettings.resize.width = 600;
        newSettings.resize.maintainAspectRatio = true;
        break;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0D0D0D] border-l border-[#222] w-80 shrink-0 overflow-y-auto custom-scrollbar">
      ...
      <div className="p-4 space-y-6 flex-1">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['Maximum Quality', 'Web Optimized', 'Extreme Compression', 'Social Media', 'Thumbnail', 'Discord/Chat'].map(p => (
              <button
                key={p}
                onClick={() => setPreset(p)}
                className="p-2 border border-[#222] bg-[#141414] hover:border-blue-500/50 hover:bg-[#1A1A1A] rounded text-[10px] text-slate-400 text-left transition-all hover:text-white"
              >
                {p}
              </button>
            ))}
      ...
      <div className="p-4 bg-[#111] border-t border-[#222] sticky bottom-0">
        ...
        <p className="text-[9px] text-[#444] text-center mt-3 uppercase tracking-tighter font-mono">
          Parallel WASM Processing Pipeline
        </p>
      </div>
    </div>
  );
