...
export const JxlSettings: React.FC<JxlSettingsProps> = ({
  ...
  return (
    <div className={`space-y-4 animate-in fade-in duration-300 ${mode === 'panel' ? 'p-3 bg-[#111] rounded-xl border border-[#222]' : ''}`}>
      <div className="space-y-3 bg-[#111] p-3 rounded-xl border border-[#222]">
        <div className="flex justify-between items-center">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Encoding Effort</span>
          <span className="text-[10px] font-mono font-black text-slate-500 bg-[#1A1A1A] px-1.5 rounded">{options.effort}</span>
        </div>
        <input
          type="range"
          min="1"
      ...
      <div className="space-y-3 bg-[#111] p-3 rounded-xl border border-[#222]">
        <div className="flex justify-between items-center">
          ...
          <span className="text-[10px] font-mono font-black text-slate-500 bg-[#1A1A1A] px-1.5 rounded">{options.quality}%</span>
        </div>
        <input
          type="range"
          min="0"
          ...
          className={`w-full h-1 rounded-lg appearance-none cursor-pointer border border-[#222] ${
            options.lossless ? 'bg-[#1A1A1A] accent-slate-600 opacity-40' : 'bg-[#1A1A1A] accent-blue-600'
          }`}
        />
        <div className="flex justify-between text-[9px] text-slate-500 uppercase font-bold tracking-widest mt-1">
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="flex items-center gap-3 cursor-pointer group bg-[#111] p-2.5 rounded-xl border border-[#222] select-none">
          ...
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight group-hover:text-slate-200 transition-colors">
            Lossless
          </span>
        </label>
