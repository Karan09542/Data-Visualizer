...
extractHook(
  /const \[collagePaddingPercent/m,
  /const \[collageBorderStyle, setCollageBorderStyle\] = useState<'solid' \| 'dashed' \| 'none'>\('dashed'\);/m,
  'useCollageConfig',
  'src/components/image-workspace/hooks/useCollageConfig.ts'
);

extractHook(
  /const \[shapeFillColor/m,
  /const \[shapeStrokeLineCap, setShapeStrokeLineCap\] = useState<'butt' \| 'round' \| 'square'>\('butt'\);/m,
  'useShapeProperties',
  'src/components/image-workspace/hooks/useShapeProperties.ts'
);
